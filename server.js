...
app.get("/country/:code", async (req, res) => {
  ...
  try {
    const countryResponse = await axios.get(`https://restcountries.com/v3/alpha/${code}`);
  ...
  try {
    ...
    <body>
        ...
        <div class="container">
          ...
          <div class="collapse navbar-collapse" id="navbarResponsive">
            <ul class="navbar-nav ml-auto">
              <li class="nav-item">
                <a class="nav-link" href="/map?city=Astana">Open the map</a>
              </li>
            </ul>
          </div>
        </div>
      ...
      <div class="container mt-4">
        <!-- Weather-Map Content -->
        <h1 class="mb-4">Weather-Map - ${cityData.name}</h1>
        
        <!-- Search Form -->
        <form id="searchForm" class="mb-3">
          <div class="input-group">
            <input type="text" class="form-control" id="cityInput" placeholder="Search on Weather-Map">
            <div class="input-group-append">
              <button class="btn btn-primary" type="submit">Explore</button>
            </div>
          </div>
        </form>
        <h2>More Information About the Location</h2>
        <p id="country-name"></p>
